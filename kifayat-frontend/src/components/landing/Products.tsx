  ...
  return (
    <section className="bg-paper py-16 lg:py-28">
      <div className="max-w-[1600px] mx-auto px-5 lg:px-10">
        ...
        {/* ── 4-col product grid ── */}
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-10 lg:gap-x-6 lg:gap-y-16">
                ...
                <Link to="/products/$productId" params={{ productId: p.slug }} className="block" data-cursor="view">
                  <div className="relative aspect-[3/4] bg-bone overflow-hidden mb-3 lg:mb-4 img-bone-grade">
                    <img
                      ...
                      width={600}
                      height={800}
                      className={`size-full object-cover transition-transform duration-700 group-hover:scale-[1.05] ${p.inStock === false ? "grayscale opacity-70" : ""}`}
                    />
                    ...
                    <span className="absolute inset-x-0 bottom-0 h-10 bg-coal text-bone eyebrow text-xs flex items-center justify-center gap-1.5 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]">
